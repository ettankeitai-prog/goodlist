...
# 目次
# ① Google フォーム作成<br>
  1) Google フォーム(https://docs.google.com/forms/u/0/) を開く。<br>
  2) 空白のフォームを選択。<br>
  3) フォーム名を入れる（なんでもいいです）<br>
  4) 以下の内容で解答欄を作成。(名前を元にデータを取得してるため基本固定)<br>
     ・質問名：画像URL 種類：ファイルのアップロード)<br>
     ・質問名：アイテムタイトル 種類：記述式(短文)<br>
     ・質問名：入手日 種類：日付<br>
     ・質問名：入手箇所 種類：プルダウン （好きな分類を追加）<br>
     ・質問名：無料/有料 種類：選択肢（無料・有料・その他）<br>
     ・質問名：コメント 種類：段落<br>
     参考用フォーム : https://docs.google.com/forms/d/e/1FAIpQLSfyB5n_uSx3erBO52Jfnmd9EawXHfpPVm-n3s34MFtFmLzswA/viewform
# ② フォルダの共有権限を変更<br>
  1) 上記で作ったフォームのファイルアップロードの質問をクリック。<br>
  2) 表示される「フォルダを表示」ボタンを押す。<br>
  3) google drive のアップロード表フォルダが表示されるので、<br>
     上にある共有ボタンを押す。（人が二人重なってるようなマーク）<br>
  4) 一般的なアクセスを "制限付き" から "リンクを知っている全員" へ変更。（閲覧者）<br>
# ③ フォームに回答を投稿しフォーム記録用のスプレッドシートを作成。<br>
  1) 上部にある "回答" をクリックし "スプレッドシートにリンク" を押す。<br>
  2) 新しいスプレッドシートを作成を選択肢作成。(スプレッドシート名は自由)<br>
  3) 作成されたスプレッドシートが以下の並びになっていることを確認。<br>
     タイムスタンプ・画像URL・アイテムタイトル・入手日・入手箇所・無料/有料・コメント<br>
# ④ フォーム記録用のスプレッドシートに Apps script を登録。<br>
  1) 開いているスプレッドシート上部の "拡張機能" - "Apps Script" をクリック。<br>
  2) 開いたコード画面に以下にある Apps script用コードを貼り付けて保存。<br>
  
# ⑥ デプロイ<br>
  1) 右上にある "デプロイ" - "新しいデプロイ" をクリック。<br>
  2) 左上種類の選択から "ウェブアプリ" をクリック。<br>
  3) 説明等自由にいれて右下のデプロイをクリック。<br>
  4) 初回のみアクセス権限の確認が入るため許可。<br>
  5) 表示されたデプロイURLが公開用ページになる。<br>
<br>
#【以下Apps script用コード】
